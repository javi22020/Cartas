...
def inferencia(imagen: Image, temp: float):
    red.eval()
    return dict(zip(cartas, [F.softmax(red.forward(t(imagen).unsqueeze(0).to(device), temp), dim=1)[0][i].item() for i in range(len(cartas))]))

gr.Interface(fn=inferencia, live=True, inputs=[gr.Image(image_mode="RGB", type="pil"), gr.Slider(minimum=0.01, maximum=2, value=1, step=0.01, label="Temperature")], outputs=gr.Label(num_top_classes=5)).launch(inbrowser=True)